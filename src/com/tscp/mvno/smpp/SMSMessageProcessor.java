...
public class SMSMessageProcessor {
	...
	public void processMessage(List<SubmitSM> smsList) throws Exception{				
		...
		try {
		    for( int j = 0; j < smsList.size(); j++ ) {
				...
				try {
			    	if(!smppConnection.isBound()){ // the smpp connection is unbound, need to bind
		...
		logger.info("Total number of messages sent out: " + messageSentCounter);
	}
	...
	private List<SubmitSM> getPromCapabilityMessageList() throws Exception{ 
		...
		spargs.put("sp", "tscp_woms_pkg.sp_get_marketing_sms");
		ResultSet rs = null;
